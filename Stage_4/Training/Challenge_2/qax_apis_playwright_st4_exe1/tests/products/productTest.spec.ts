...
import { generateRandomProduct } from '../../src/helpers/dataBuilder';

test.describe.serial('Products API con TypeScript y data dinamica @products', () => {
    ...
    let productToCreate!: ProductRequestBody; // guardo la data dinamica que se enviara en el POST

    test.beforeEach(async ({ request }) => {
        productService = new ProductService(request); // antes de cada test inicializo el service
    });

    test('debe crear un producto con data dinamica usando POST @post @smoke', async () => {
        console.log('========== INICIO DEL TEST POST ==========');
        console.log('Se inicia la prueba para crear un producto con data dinamica generada con Faker.');

        // ARRANGE
        // preparo la data fija que se enviara en el body del POST
        // ahora ya no se prepara data fija, ahora se genera data dinamica con Faker
        console.log('ARRANGE: Se prepara la data dinamica que se enviara para crear el producto.');
        ...
        productToCreate = generateRandomProduct(); // asigno la data random creada

        console.log('ARRANGE: La data dinamica que se enviara para crear el producto es:', productToCreate);
        ...
        console.log(
            'Se ha creado correctamente el producto con data dinamica. El ID generado es:',
    ...
    test('debe consultar el producto creado usando GET @get @smoke', async () => {
        console.log('========== INICIO DEL TEST GET ==========');
